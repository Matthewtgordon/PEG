...
class APEGOrchestrator:
    ...
    - Supports dynamic agent registration and plugin loading
    """
    ...
    def __init__(
        ...
        Args:
            ...
            enable_plugins: Whether to load plugins from plugin_dir
        ...
        logger.info("Dynamic agents: %d, Plugins: %d",
                    len(self.agents),
                    len(self.plugin_mgr.plugins) if self.plugin_mgr else 0)
    ...
    def _initialize_plugins(self) -> None:
        ...
        try:
            from apeg_core.connectors.plugin_manager import PluginManager
            ...
            if self._plugin_dir.exists():
                loaded = self.plugin_mgr.load_plugins()
                logger.info("Loaded %d plugins: %s", len(loaded), loaded)
            else:
                logger.debug("Plugin directory does not exist: %s", self._plugin_dir)

        except Exception as e:
            logger.warning("Failed to initialize plugins: %s", e)
            self.plugin_mgr = None

    def add_agent(self, name: str, impl: str) -> bool:
        """
        Dynamically add an agent from implementation code.

        This method enables runtime addition of agents generated by
        the MetaAgent. The implementation should define an AgentClass
        that can be instantiated.

        Args:
            name: Unique agent identifier
            impl: Python code string containing agent implementation

        Returns:
            True if agent was added successfully
        """
        logger.info("Adding dynamic agent: %s", name)
        ...
        try:
            ...
            # Execute implementation in module context
            exec(impl, mod.__dict__)

            # Look for AgentClass or agent_execute function
            if hasattr(mod, "AgentClass"):
                self.agents[name] = mod.AgentClass()
                logger.info("Added agent %s with AgentClass", name)
                return True
            elif hasattr(mod, "agent_execute"):
                # Wrap function as callable agent
                self.agents[name] = mod.agent_execute
                logger.info("Added agent %s with agent_execute function", name)
                return True
            else:
                logger.error("Agent %s has no AgentClass or agent_execute", name)
                return False

        except Exception as e:
            logger.error("Failed to add dynamic agent %s: %s", name, e)
            return False

    def get_dynamic_agent(self, name: str) -> Optional[Any]:
        """
        Get a dynamically added agent by name.
    ...
    def list_dynamic_agents(self) -> List[str]:
        """List all dynamically added agent names."""
    ...
    def execute_plugin(
        ...
        """
        Execute an action on a loaded plugin.
        ...
        Raises:
            RuntimeError: If plugins not initialized
        """
        if not self.plugin_mgr:
            raise RuntimeError("Plugin manager not initialized")

        return self.plugin_mgr.execute(plugin_name, action, params)

    def list_plugins(self) -> List[str]:
        """List all loaded plugin names."""
        if not self.plugin_mgr:
            return []
        return self.plugin_mgr.list_plugins()
